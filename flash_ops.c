...
void flash_write_safe(uint32_t offset, const uint8_t *data) {
    ...
    if (flash_offset + 4096 > FLASH_TARGET_OFFSET + FLASH_SIZE || flash_offset < FLASH_TARGET_OFFSET) {
    ...
    // Write data to flash
    flash_range_program(flash_offset, data, 4096);
...
void flash_read_safe(uint32_t offset, uint8_t *buffer) {
    ...
    memcpy(buffer, (void *)(XIP_BASE + flash_offset), 4096);
}
